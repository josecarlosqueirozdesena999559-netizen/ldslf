...
class Bullex:
    ...
    def __get_digital_open(self):
        # for digital options
        response = self.get_digital_underlying_list_data() or {}
        digital_data = response.get("underlying", [])
...
##############################################################################################
    ...
    def check_win_v4(self, id_number):
        ...
        try:
            ...
            profit = 0 if msg["win"] == "equal" else float(msg["sum"]) * -1 if msg["win"] == "loose" else float(msg["win_amount"]) - float(msg["sum"])
        return msg["win"], profit
